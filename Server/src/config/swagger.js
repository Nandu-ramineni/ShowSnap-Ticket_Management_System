      ...
      schemas: {
        ...
        UserPublic: {
          ...
          properties: {
            ...
            accountStatus:   { type: 'string', enum: ['pending', 'active', 'rejected'], example: 'active', description: 'Approval lifecycle (theatre_owner only)' },
            isActive:        { type: 'boolean', example: true, description: 'Account suspension flag' },
            rejectionReason: { type: 'string', nullable: true, description: 'Populated when accountStatus is rejected' },
          },
        },
        RegisterRequest: {
          type: 'object',
        ...
        // ─── TheatreOwner schemas ──────────────────────────────
        // ── TheatreOwner shared sub-schemas ──────────────────────────────────────

        SupportingDocument: {
          type: 'object',
          description: 'A document uploaded by the theatre owner during registration or later',
          properties: {
            id:         { $ref: '#/components/schemas/MongoId' },
            docType: {
              type: 'string',
              enum: ['gst_certificate', 'business_registration', 'trade_license',
                     'pan_card', 'identity_proof', 'address_proof', 'noc', 'other'],
              example: 'gst_certificate',
            },
            url:        { type: 'string', format: 'uri', example: 'https://res.cloudinary.com/seatsecure/...' },
            fileName:   { type: 'string', example: 'gst_certificate.pdf' },
            fileSize:   { type: 'integer', example: 204800, description: 'File size in bytes' },
        ...
        TheatreInfo: {
          type: 'object',
          properties: {
            theatreName:  { type: 'string',  example: 'INOX Multiplex', maxLength: 150 },
            website:      { type: 'string',  format: 'uri', example: 'https://inoxmovies.com' },
            contactPhone: { type: 'string',  example: '+918800123456' },
            contactEmail: { type: 'string',  format: 'email', example: 'contact@inoxcinemas.com' },
          },
        },
        TheatreOwnerLocation: {
          type: 'object',
          properties: {
            streetAddress: { type: 'string', example: '42 MG Road, Indiranagar', maxLength: 200 },
            city:          { type: 'string', example: 'Bengaluru',  maxLength: 100 },
            state:         { type: 'string', example: 'Karnataka',  maxLength: 100 },
            pincode:       { type: 'string', example: '560038', pattern: '^\\d{4,10}$' },
          },
        },
        Amenities: {
          type: 'object',
        ...
        CancellationPolicy: {
          ...
          properties: {
            ...
            cutoffHours:        { type: 'number',  minimum: 0, maximum: 720, default: 2,  example: 3,  description: 'Hours before showtime within which cancellation is permitted' },
            refundPercentage:   { type: 'number',  minimum: 0, maximum: 100, default: 0,  example: 75, description: 'Percentage of ticket price refunded on cancellation' },
          },
        },

        // ── TheatreOwner composite schemas ────────────────────────────────────

        TheatreOwnerPublic: {
          type: 'object',
          description: 'Theatre owner document — public fields only (no password, no Cloudinary publicIds)',
          properties: {
            id:    { $ref: '#/components/schemas/MongoId' },
            email: { type: 'string', format: 'email', example: 'priya@inoxcinemas.com' },
            name:  { type: 'string', example: 'Priya Nair' },
            supportingDocuments: {
              type:  'array',
              items: { $ref: '#/components/schemas/SupportingDocument' },
              description: 'Documents uploaded during registration',
            },
            isMultiplex:         { type: 'boolean', example: true },
            theatreInfo:         { $ref: '#/components/schemas/TheatreInfo' },
            location:            { $ref: '#/components/schemas/TheatreOwnerLocation' },
            ...
            accountStatus: {
              ...
              example: 'active',
              description: 'Admin approval lifecycle',
            },
            onboardingStatus: {
              type:    'string',
              enum:    ['pending_onboarding', 'in_progress', 'completed'],
              example: 'pending_onboarding',
              description: 'Post-approval profile completion lifecycle',
            },
        ...
        // Step 1 — register (multipart — documented inline in route)
        TheatreOwnerRegisterRequest: {
          type: 'object',
          required: ['email', 'password', 'name'],
          description: 'multipart/form-data — see POST /theatre-owner/register for full spec',
          properties: {
            email:    { type: 'string', format: 'email', example: 'priya@inoxcinemas.com' },
            password: { type: 'string', minLength: 8,   example: 'Owner@123' },
            name:     { type: 'string', example: 'Priya Nair' },
            documents: {
              type:        'array',
            ...
            docTypes: {
              ...
              description: 'One docType per uploaded document file (same order as files)',
            },
          },
        },

        // Step 3 — onboarding wizard
        TheatreOwnerOnboardingRequest: {
          type: 'object',
          description: 'Partial saves allowed — all fields optional individually. Required for completion: theatreInfo.theatreName, contactPhone, contactEmail, full location.',
          properties: {
            name:        { type: 'string', example: 'Priya Nair', maxLength: 150 },
            isMultiplex: { type: 'boolean', example: true },
            theatreInfo: {
              type: 'object',
              properties: {
                theatreName:  { type: 'string', example: 'INOX Multiplex', description: 'Required for onboarding completion' },
                website:      { type: 'string', format: 'uri', example: 'https://inoxmovies.com' },
                contactPhone: { type: 'string', example: '+918800123456', description: 'Required for onboarding completion' },
                contactEmail: { type: 'string', format: 'email', example: 'contact@inoxcinemas.com', description: 'Required for onboarding completion' },
              },
            },
            location: {
              type: 'object',
              description: 'All four fields required for onboarding completion',
              properties: {
                streetAddress: { type: 'string', example: '42 MG Road, Indiranagar' },
        ...
        TheatreOwnerUpdateRequest: {
          type: 'object',
          description: 'Partial update — include only fields to change. theatreInfo.theatreName is immutable.',
          properties: {
            name:        { type: 'string', example: 'Priya Krishnan' },
            isMultiplex: { type: 'boolean' },
            theatreInfo: {
              type: 'object',
              description: 'theatreName is immutable after registration — contact support to rename',
              properties: {
        ...
        // Login response
        TheatreOwnerAuthTokens: {
          type: 'object',
          properties: {
            owner:        { $ref: '#/components/schemas/TheatreOwnerPublic' },
            accessToken:  { type: 'string', example: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...' },
            refreshToken: { type: 'string', example: 'a3f9c2e1b4d7...' },
          },
        },
      },
